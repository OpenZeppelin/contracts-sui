# Contributing to OpenZeppelin Contracts for Sui
...
## AI integrator metadata

Each package ships machine-readable metadata for AI integrator agents under `<pkg>/llms/` (per-module YAMLs + an `index.yaml`), discoverable via the root `llms.txt` and `llms/index.yaml` catalog. It is generated by the `generate-sui-metadata` skill (`.claude/skills/generate-sui-metadata/`).

CI validates three layers on every PR: JSON Schema conformance, cross-field semantic integrity, and **completeness** (every public source module has a YAML + index entry; no orphans). CI does **not** check content drift — i.e. it will not catch a module YAML whose `api` / `errors` / `types` have gone stale relative to the source.

Therefore:

- **When a PR changes a module's public surface** (adds/removes/renames a function, error, or type, or changes a signature), **regenerate that module's YAML** with the skill so the metadata stays accurate. This is a PR checklist item.
- **Release process requirement:** before tagging a release, **re-run `generate-sui-metadata` for every module** and commit any diffs. This is the safety net that closes content drift at the release boundary — the metadata a release publishes must match the released code.
