...
4. **Inspect** - read allowance state on-chain without mutating: `allowance<T>` (the configured budget), `spendable_now<T>` (what a spend would allow right now, after expiry and the pool balance), `expiry<T>`, `contains<T>`, and `balance_value<T>` (the pooled balance of a coin type).
