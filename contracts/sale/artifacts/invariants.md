...
- **Per-buyer-cap `contributions` table pruning** — never pruned, by design. For a
  sale with a per-buyer cap, `contributions: Table<address, u64>` grows one
  buyer-funded entry per distinct buyer and is never reclaimed (the sale object is
  itself permanent). Access is O(1), so there is no liveness/DoS risk; the growth
  is an accepted storage cost. Uncapped sales leave the table `None`.
...
3. ~~Per-buyer cap state growth — accepted?~~ **Resolved (2026-06-25): accepted by
   design.** `contributions` is never pruned; see Out of Scope. No invariant change.
