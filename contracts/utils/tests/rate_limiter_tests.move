...
// === Cooldown reconfigure rebases in-flight deadline to new cooldown_ms ===

#[test]
fun cooldown_reconfigure_rebases_in_flight_deadline_on_grow() {
    let (test, mut clk) = setup(0);

    let mut rl = rate_limiter::new_cooldown(1, 50, 1);
    assert!(rl.try_consume(1, &clk)); // cooldown_end_ms = 50, available = 0, arm_time = 0

    // Reconfigure mid-cooldown with a longer cooldown. The deadline is rebased as if the
    // new cooldown_ms had been in effect since the gate was armed:
    // new_deadline = arm_time + new_cooldown_ms = 0 + 100 = 100.
    clk.set_for_testing(20);
    rl.reconfigure_cooldown(1, 100, &clk);

    // Just before the rebased deadline: still gated.
    clk.set_for_testing(99);
    assert_eq!(rl.available(&clk), 0);

    // At the rebased deadline: gate releases.
    clk.set_for_testing(100);
    assert_eq!(rl.available(&clk), 1);
    assert!(rl.try_consume(1, &clk)); // arms a fresh gate with NEW cooldown_ms=100

    // The fresh gate uses the new cooldown: 100 + 100 = 200.
...
#[test]
fun cooldown_reconfigure_rebases_in_flight_deadline_on_shrink() {
    let (test, mut clk) = setup(0);
    ...
    // 200 ms later, shrink cooldown to 500. Rebased deadline = arm_time + new = 0 + 500.
    clk.set_for_testing(200);
...
fun cooldown_reconfigure_shrink_past_now_releases_immediately() {
    ...
    // At t=500, shrink cooldown to 100. Rebased deadline = 0 + 100 = 100 <= 500.
    // Gate releases immediately and `available` becomes the new capacity.
    clk.set_for_testing(500);
...
fun cooldown_reconfigure_with_identical_cooldown_preserves_deadline() {
    ...
    // At the original deadline: gate releases (deadline was NOT pushed to 150).
    clk.set_for_testing(100);
    assert_eq!(rl.available(&clk), 1);
