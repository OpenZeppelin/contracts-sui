...
/// Generic macro for computing 10^exp for any integer type.
///
/// **Important**: This macro does NOT validate the exponent.
/// The caller MUST validate that `exp <= MAX_POW_10_EXPONENT` before calling.
///
/// **Security Note**: If called with an invalid exponent, this function will abort
/// on arithmetic overflow during the multiplication loop, which is the intended behavior.
/// The macro relies on the caller's validation to prevent this.
///
/// # Type Parameters
///
/// * `$Int` - The target integer type (`u8`, `u16`, `u32`, `u64`, `u128`, or `u256`).
///
/// # Arguments
///
/// * `$exp` - Exponent value (must be pre-validated by caller).
///
/// # Returns
///
/// 10^exp as type $Int.
...
/// # Aborts
///
/// Aborts on arithmetic overflow if `exp` exceeds the type's maximum supported exponent.
/// This is expected behavior when the macro is called without proper validation.
public(package) macro fun pow_10<$Int>($exp: u8): $Int {
    let exp = $exp;
    ...
    // General case: `exp` >= 2.
    let mut result = 100 as $Int;
    let mut i = 2u8;
    while (i < exp) {
        result = result * (10 as $Int);
        i = i + 1;
    };
    result
}
