...
fun quick_sort_empty_vector() {
    let data: vector<u128> = vector[];
    let sorted = u128::quick_sort(data);
    assert_eq!(sorted.length(), 0);
}

#[test]
fun quick_sort_single_element() {
    let data = vector[42u128];
    let sorted = u128::quick_sort(data);
    assert_eq!(sorted.length(), 1);
    assert_eq!(sorted[0], 42u128);
}

#[test]
fun quick_sort_already_sorted() {
    let data = vector[1u128, 2u128, 3u128, 4u128, 5u128];
    let sorted = u128::quick_sort(data);
    assert_eq!(sorted[0], 1u128);
    assert_eq!(sorted[1], 2u128);
    assert_eq!(sorted[2], 3u128);
    assert_eq!(sorted[3], 4u128);
    assert_eq!(sorted[4], 5u128);
}

#[test]
fun quick_sort_reverse_sorted() {
    let data = vector[5u128, 4u128, 3u128, 2u128, 1u128];
    let sorted = u128::quick_sort(data);
    assert_eq!(sorted[0], 1u128);
    assert_eq!(sorted[1], 2u128);
    assert_eq!(sorted[2], 3u128);
...
fun quick_sort_unsorted() {
    let data = vector[3u128, 1u128, 4u128, 1u128, 5u128, 9u128, 2u128, 6u128];
    let sorted = u128::quick_sort(data);
    assert_eq!(sorted[0], 1u128);
    assert_eq!(sorted[1], 1u128);
    assert_eq!(sorted[2], 2u128);
    assert_eq!(sorted[3], 3u128);
    assert_eq!(sorted[4], 4u128);
    assert_eq!(sorted[5], 5u128);
    assert_eq!(sorted[6], 6u128);
    assert_eq!(sorted[7], 9u128);
}

#[test]
fun quick_sort_duplicates() {
    let data = vector[5u128, 2u128, 5u128, 1u128, 2u128, 5u128];
    let sorted = u128::quick_sort(data);
    assert_eq!(sorted[0], 1u128);
    assert_eq!(sorted[1], 2u128);
    assert_eq!(sorted[2], 2u128);
    assert_eq!(sorted[3], 5u128);
    assert_eq!(sorted[4], 5u128);
    assert_eq!(sorted[5], 5u128);
}
...
fun quick_sort_all_same() {
    let data = vector[7u128, 7u128, 7u128, 7u128];
    let sorted = u128::quick_sort(data);
    assert_eq!(sorted[0], 7u128);
    assert_eq!(sorted[1], 7u128);
    assert_eq!(sorted[2], 7u128);
    assert_eq!(sorted[3], 7u128);
}

#[test]
fun quick_sort_large_numbers() {
    let data = vector[
        18446744073709551615u128,  // Large u128 value
        1u128,
    ...
    let sorted = u128::quick_sort(data);
    assert_eq!(sorted[0], 1u128);
    assert_eq!(sorted[1], 5000000000u128);
    assert_eq!(sorted[2], 1000000000000000000u128);
    assert_eq!(sorted[3], 18446744073709551615u128);
}
