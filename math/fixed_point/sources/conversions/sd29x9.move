...
/// `sd29x9::wrap`, `sd29x9::from_bits`, and `casting_u128::into_SD29x9`, which
...
public fun from_u128(x: u128, is_negative: bool): SD29x9 {
    assert!(x <= fixed_point_common::max_sd29x9_whole(), EOverflow);
    sd29x9::wrap(x * fixed_point_common::scale(), is_negative)
}
...
public fun try_from_u128(x: u128, is_negative: bool): Option<SD29x9> {
    if (x > fixed_point_common::max_sd29x9_whole()) {
        option::none()
    } else {
        option::some(sd29x9::wrap(x * fixed_point_common::scale(), is_negative))
    }
}
...
///
/// The sign flag is always `false` when the result is zero.
public fun to_parts_trunc(x: SD29x9): (u128, bool) {
    let bits = x.unwrap();
    let (neg, mag) = if ((bits & fixed_point_common::sign_bit()) != 0) {
        (true, two_complement(bits))
    } else {
        (false, bits)
    };
    let whole = mag / fixed_point_common::scale();
    (whole, neg && (whole != 0))
}
...
public fun to_u128_trunc(x: SD29x9): u128 {
    let (whole, is_negative) = to_parts_trunc(x);
    assert!(!is_negative, ENegativeValue);
    whole
}
