...
/// `ud30x9::wrap` and `casting_u128::into_UD30x9`, which preserve the raw
...
public fun from_u128(x: u128): UD30x9 {
    assert!(x <= fixed_point_common::max_ud30x9_whole(), EOverflow);
    ud30x9::wrap(x * fixed_point_common::scale())
}
...
public fun try_from_u128(x: u128): Option<UD30x9> {
    if (x > fixed_point_common::max_ud30x9_whole()) {
        option::none()
    } else {
        option::some(ud30x9::wrap(x * fixed_point_common::scale()))
    }
}
...
public fun to_u128_trunc(x: UD30x9): u128 {
    x.unwrap() / fixed_point_common::scale()
}
