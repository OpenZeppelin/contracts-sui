...
fun pow_alt_handles_zero_and_one_exponents() {
    let x = fixed(12 * SCALE + 345_678_901);
    expect(x.pow_alt(0), fixed(SCALE));
    expect(x.pow_alt(1), x);
    expect(fixed(0).pow_alt(0), fixed(SCALE));
}

#[test]
fun pow_alt_handles_zero_and_one_bases() {
    expect(fixed(0).pow_alt(5), fixed(0));
    expect(fixed(SCALE).pow_alt(17), fixed(SCALE));
}
...
fun pow_alt_handles_fractional_values_and_truncation() {
    // 1.5^2 = 2.25, 1.5^3 = 3.375
    let one_point_five = fixed(1_500_000_000);
    expect(one_point_five.pow_alt(2), fixed(2_250_000_000));
    expect(one_point_five.pow_alt(3), fixed(3_375_000_000));

    // 1.000000001^2 = 1.000000002000000001 -> 1.000000002
    let epsilon = fixed(SCALE + 1);
    expect(epsilon.pow_alt(2), fixed(SCALE + 2));
}
