...
fun pow_handles_zero_and_one_exponents() {
    ...
    assert_eq!(x.pow(0).unwrap(), fixed(SCALE).unwrap());
    assert_eq!(x.pow(1).unwrap(), x.unwrap());
    assert_eq!(fixed(0).pow(0).unwrap(), fixed(SCALE).unwrap());
}

#[test]
fun pow_handles_zero_and_one_bases() {
    assert_eq!(fixed(0).pow(5).unwrap(), fixed(0).unwrap());
    assert_eq!(fixed(SCALE).pow(17).unwrap(), fixed(SCALE).unwrap());
}
...
fun pow_handles_fractional_values_and_truncation() {
    ...
    assert_eq!(one_point_five.pow(2).unwrap(), fixed(2_250_000_000).unwrap());
    assert_eq!(one_point_five.pow(3).unwrap(), fixed(3_375_000_000).unwrap());

    // 1.000000001^2 = 1.000000002000000001 -> 1.000000002
    let epsilon = fixed(SCALE + 1);
    assert_eq!(epsilon.pow(2).unwrap(), fixed(SCALE + 2).unwrap());
}
...
fun pow_supports_high_exponents() {
    ...
    assert_eq!(pow255.unwrap(), fixed(5_152_918_999_790_606_401_120_741_084_983_548).unwrap());
...
fun pow_two_squared() {
    // 2.0^2 = 4.0
    assert_eq!(fixed(2 * SCALE).pow(2).unwrap(), fixed(4 * SCALE).unwrap());
}

#[test]
fun pow_three_cubed() {
    // 3.0^3 = 27.0
    assert_eq!(fixed(3 * SCALE).pow(3).unwrap(), fixed(27 * SCALE).unwrap());
}

#[test]
fun pow_half_squared() {
    // 0.5^2 = 0.25
    assert_eq!(fixed(500_000_000).pow(2).unwrap(), fixed(250_000_000).unwrap());
}
