...
use openzeppelin_collections::random;
...
fun new_list_for_test<V: store + copy + drop>(
    ...
    let mut list = skip_list::new<u64, V>(max_leveL, list_p, seed, ctx);
...
fun test_new() {
    ...
    let list = skip_list::new<u64, u256>(16, 2, 12345, ctx);
    list.check_skip_list!();
...
fun test_borrow() {
    let ctx = &mut tx_context::dummy();
    let mut list = skip_list::new<u64, u256>(16, 2, 12345, ctx);
    list.insert!(1, 1001);
    list.insert!(5, 1002);
    list.insert!(6, 1003);
    list.insert!(7, 1004);
...
fun test_find_in_empty_list() {
    let ctx = &mut tx_context::dummy();
    let list = skip_list::new<u64, u256>(16, 2, 1234, ctx);
...
fun test_rand_level() {
    let ctx = &mut tx_context::dummy();
    let mut list = skip_list::new<u64, u256>(2, 2, 0, ctx);
    list.insert!(1, 1001);
    list.insert!(2, 1002);
    list.insert!(3, 1003);
    list.insert!(4, 1004);
