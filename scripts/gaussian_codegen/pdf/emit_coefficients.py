...
def emit_module(num: list[tuple[int, bool]], den: list[tuple[int, bool]]) -> str:
    ...
    wad_exp = len(str(constants.PDF_WAD)) - 1  # power-of-ten exponent; tracks the constant

    return f"""{banner}
/// Numerator and denominator coefficients for the rational `N(z) / D(z)`
/// standard-normal PDF approximation on the central domain `[0, {constants.PDF_MAX_Z}]`.
...
/// sign-magnitude pairs at PDF WAD (`10^{wad_exp}`) scale, indexed in ascending power
