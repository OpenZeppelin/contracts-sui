"""Emit test-vector Move files for both `sd29x9_base::pdf` and `ud30x9_base::pdf`.
...
from gaussian_codegen.shared.reference import DPS, SCALE_DECIMAL, pdf as pdf_oracle
...
def expected_pdf_raw(z_str: str) -> int:
    """φ(±z) at the UD30x9 raw scale. φ is even, so the sign is irrelevant.
    ...
    *quantized* input `z_raw` meets or exceeds `MAX_Z_RAW` (the `>=` test in
    ...
    if quantize_z(z_str) >= MAX_Z_RAW:
        return 0
    return int(pdf_oracle(mpf(z_str)) * mpf(SCALE_DECIMAL) + mpf("0.5"))


def build_test_cases() -> list[tuple[int, bool, int]]:
    """Build (z_raw, neg, expected_pdf_raw) tuples, deduplicated and sorted."""
    seen: set[tuple[int, bool]] = set()
    cases: list[tuple[int, bool, int]] = []
    for z_str in CRITICAL_Z + evenly_spaced_z():
        z_raw = quantize_z(z_str)
        expected = expected_pdf_raw(z_str)
        for neg in (False, True):
            ...
            if key in seen:
                continue
            seen.add(key)
            cases.append((z_raw, neg, expected))
...
def emit_sd29x9_module(cases: list[tuple[int, bool, int]]) -> str:
    banner = auto_generated_banner(
        "scripts/gaussian_codegen/pdf/emit_test_vectors.py (oracle: mpmath npdf at 100 dps)"
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Emit pdf test vector Move files for SD29x9 and UD30x9."
    )
    ...
    if args.check:
        ...
        if ok:
            ...
            return 0
        print("FAIL: run `python -m gaussian_codegen.pdf.emit_test_vectors` to regenerate", file=sys.stderr)
        return 1
