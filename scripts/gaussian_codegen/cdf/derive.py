"""Derive the standard-normal CDF rational by optimizing the exact integer
pipeline (PR2), not a continuous float64 fit.

Through PR1 this fit a SciPy AAA rational against float64 `norm.cdf` and accepted
the smallest degree whose *continuous* error met a budget - so the coefficients
were optimal for the continuous rational, never for the quantized integer
function that actually ships. This rewrite instead:

- seeds from the AAA rational (a strong near-minimax start, retained as a
  cross-check) and an independent linearized-minimax seed;
- refines each with a descending-`p` IRLS homotopy toward the correctly-rounded
  count (`shared/fit.py`);
- scores every candidate by running it through the bit-identical integer mirror
  (`shared/arithmetic.py`) against the 100-dps mpmath oracle and counting
  correctly-rounded outputs (`shared/accuracy.py`);
- selects the strictly-monotone candidate with the fewest misses
  (`shared/refit.py`).

Degree is pinned to TARGET_DEGREE (held fixed across regenerations so a
precision/domain change never silently moves it). Coefficients are ascending
power order, `D(0) = 1`. Pass `--report` for the full candidate table.
"""
...
N_FIT_GRID = 5000
# Descending-p IRLS homotopy from each seed; selection keeps the best monotone.
P_SCHEDULE = [1.0, 0.85, 0.72, 0.6, 0.5, 0.42]
PIN_ZERO_SLOPE = False  # Phi'(0) = phi(0) != 0, so no even-peak pin (unlike PDF)
INCREASING = True
...
def aaa_seed() -> tuple[list[mpf], list[mpf]]:
    ...
    grid = np.linspace(0.0, float(constants.MAX_Z), N_FIT_GRID)
    aaa = AAA(grid, norm.cdf(grid), max_terms=TARGET_DEGREE + 1, rtol=1e-15, clean_up_tol=0)
...
def assert_signs_central(num: Sequence[mpf], den: Sequence[mpf], n: int = 10000) -> None:
    """Verify D(z) > 0 and N(z) >= 0 on the central domain (INV-7 / INV-8)."""
    mp.dps = DPS
    for i in range(n + 1):
        z = MAX_Z * mpf(i) / n
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Derive CDF rational (exact-pipeline objective).")
    ...
    output = {
        ...
        "method": "AAA seed + descending-p IRLS homotopy, selected by exact-pipeline correctly-rounded count",
        "selected": best.tag,
